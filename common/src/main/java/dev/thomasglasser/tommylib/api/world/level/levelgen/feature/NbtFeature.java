...
public class NbtFeature extends Feature<NbtFeatureConfig> {
    ...
    public boolean place(FeaturePlaceContext<NbtFeatureConfig> context) {
        ...
        if (context.config().nbts().isEmpty()) {
            return false;
        }
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                if ((x * x) + (z * z) < radius * radius + 1) {
                    ...
                    // No breaking blocks unless config allows
                    else if (context.level().getBlockState(blockpos$Mutable.move(Direction.UP)).canOcclude() && !context.config().canBreakExistingBlocks()) {
                        return false;
                    }
                    // No spawning in air
                    else if (!context.level().getBlockState(blockpos$Mutable.move(Direction.DOWN, 3)).canOcclude()) {
                        return false;
                    }
                }
            }
